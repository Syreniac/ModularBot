...
class ModularBot(sleekxmpp.ClientXMPP):
	def __init__(self,jid,password,nick,channel,excluded,tiebreaker,server,admins,banned_subreddits,reddit_password,bot_id,plugin_status={}):
		...
		self.banned_subreddits=banned_subreddits
	...
	def updateJIDs(self,msg):
		...
		if msg["type"]!="unavailable":
			if not msg["mucnick"] in self.jidList.keys():
				self.jidList[nick]=msg["muc"]["jid"].bare
	...
	def msg_handler(self,msg):
		...
		if msg["type"]=="groupchat":
			ignored=msg["mucnick"].lower() in self.excluded
		else:
			ignored=False
